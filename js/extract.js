...
(function (FC) {
  ...
  const RULES = [
    ...
    // (?<!под) — иначе «форма ПОДБОРОДка», «округлый ПОДБОРОДок» ложно матчат
    // «бород» как подстроку (растительность вместо формы подбородка).
    [/эспаньол|козлин\w*\s+бород|(?<!под)бородк/i, 'facialHair', 'goatee'],
    [/(?<!под)борода|(?<!под)бород\w/i, 'facialHair', 'shortBeard', 'med'],
    ...
    // (?<![а-яёА-ЯЁ]) — граница слова: без нeё «кусочки», «точки», «цепочки»
    // и любое другое слово на «-очки» ложно матчило бы очки (простой \b тут
    // не работает — см. комментарий в cyr() про \w и кириллицу).
    [/(?<![а-яёА-ЯЁ])очк(и|ах|ов)/i, 'glasses', 'rounded', 'med'],
  ];
  ...
  function cyr(re) {
    ...
    // Разрешить пару промежуточных слов перед ключевым существительным
    // («тёмные короткие волосы», «длинные волнистые русые волосы»):
    // \s+волос -> гибкий зазор. 40 символов ~ два коротких прилагательных.
    s = s.replace(/\\s\+(волос|лиц|глаз|бров|нос|губ|подбород|уш)/g, '[а-яё ]{1,40}$1');
