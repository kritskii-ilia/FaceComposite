...
Схема берётся из traits.js. Удобно сгенерировать её рядом (traits.js вешает
FC на window, поэтому в Node нужен глобальный window-заглушка):
    node -e "global.window={};const fs=require('fs');
             eval(fs.readFileSync('js/traits.js','utf8'));
             process.stdout.write(JSON.stringify(window.FC.traits.nluSchema()))" > schema.json
